...
const triggerAPI = require("./services/triggerAPI")
...
const analyzeJsonFile = async (filePath) => {
  // Read the JSON file
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Hãy phân tích tài liệu này bao gồm địa chỉ, diện tích, mục đích sử dụng.Hãy viết Tiếng Việt\n\n${JSON.stringify(jsonContent, null, 2)}`,
      },
    ],
  });
...
app.post('/analyze-image', async (req, res) => {
  try {
    ...
    // Delete old result files after a successful download
    deleteAllFiles(resultsDir);

    // Perform text detection
    const [result] = await client.textDetection(imagePath);
    const detections = result.textAnnotations;

    const resultFilePath = path.join(resultsDir, 'result.json');
    fs.writeFileSync(resultFilePath, JSON.stringify(detections, null, 2));
    console.log(`Result saved to: ${resultFilePath}`);

    // Analyze JSON file with OpenAI
    const analysisResult = await analyzeJsonFile(resultFilePath);
    console.log(analysisResult)

    res.json({ detections, analysisResult });
  } catch (error) {
    console.error(error);
